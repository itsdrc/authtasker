...
export const rolesMiddlewareFactory = (
    ...
    return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (!payload) {
                loggerService.error('Token sent is not generated by this server');
                res.status(HTTP_STATUS_CODE.UNAUTHORIZED).json({ error: invalidBearerTokenMessageToClient });
                return;
            }
            ...
            if (tokenPurpose !== TOKEN_PURPOSES.SESSION) {
                loggerService.error(`Token purpose "${tokenPurpose}" is not the expected`);
                res.status(HTTP_STATUS_CODE.UNAUTHORIZED).json({ error: invalidBearerTokenMessageToClient });
                return;
            }
            ...
            if (canAccess(minRoleRequired, user.role)) {
                loggerService.info(`Access granted for ${userId} (${user.role})`);
                ...
                loggerService.error(`Access denied for ${userId} (${user.role})`);
